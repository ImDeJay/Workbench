...
/**
 * A fluent element mapper that relies on code, not
 * text.
 *
 * @author afistofirony
 */
public class FluentMapper extends Mapper {
    ...
    private ArrayList<ElementNode> calculateClosers () {
        ...
        for (ElementNode node : this.latestModifiers) {
            if (node.isOpening() && !persist.contains(node.getType()))
                result.add(new ElementNode(node.getType(), null, node.getID(), true));
        }
    ...
    private void removeSelectively () {
        for (ElementNode node : this.latestModifiers)
            if (!persist.contains(node.getType()))
    ...
    public FluentMapper style (ChatColor... styles) {
        ...
        for (ChatColor colour : styles) {
            if (colour.isFormat()) {
                ...
                if (!this.contains(type))
                    this.latestModifiers.add(new ElementNode(type, null, index++));
          } else {
        ...
        if (!invalidTypes.isEmpty())
            this.colour(invalidTypes.toArray(new ChatColor[invalidTypes.size()]));
